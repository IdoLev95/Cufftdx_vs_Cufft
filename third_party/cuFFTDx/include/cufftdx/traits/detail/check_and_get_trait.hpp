    ...
    namespace detail {
        ...
        namespace get_block_config_impl {
            ...
            class helper
            {
                ...
                // BlockDim is supported with CUDA 12.1 and later
                #if (__CUDACC_VER_MAJOR__ == 12) && (__CUDACC_VER_MINOR__ < 1)
                static_assert(!has_block_dim_defined, "BlockDim is not supported with CUDA 12.0");
                #endif

                // If LTO is defined, we need to check if it's supported
                static constexpr bool is_lto_defined = experimental::code_type_of<Description>::value == experimental::code_type::ltoir;
                static_assert(!(is_lto_defined && has_block_dim_defined), "BlockDim is not supported with experimental::code_type::ltoir. Use experimental::code_type::ptx instead.");
                ...
                using optimal_block_config_t = typename database::detail::query_database<ptx_backend.size,
                                                                                         ptx_backend.type,
                ...
                using selected_block_config_t = typename database::detail::query_database<ptx_backend.size,
                                                                                          ptx_backend.type,
                                                                                          ptx_backend.direction,
                                                                                          ptx_backend.sm,
                                                                                          ptx_backend.elements_per_thread,
