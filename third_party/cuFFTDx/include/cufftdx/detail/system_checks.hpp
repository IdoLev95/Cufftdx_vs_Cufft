...
// Unless explicitly directed otherwise, we  target architecture to be Volta+ (only checking on
...
#            error cuFFTDx requires NVRTC from CUDA Toolkit 12.0 or newer
#        endif
#    endif // CUFFTDX_IGNORE_DEPRECATED_COMPILER

// NVRTC compilation checks
#    ifndef CUFFTDX_IGNORE_DEPRECATED_COMPILER
static_assert((__CUDACC_VER_MAJOR__ >= 12),
              "cuFFTDx requires CUDA Runtime 12.0 or newer to work with NVRTC");
#    endif // CUFFTDX_IGNORE_DEPRECATED_COMPILER
...
static_assert(CUDART_VERSION >= 12000, "cuFFTDx requires CUDA Runtime 12.0 or newer");
static_assert(CUDA_VERSION >= 12000, "cuFFTDx requires CUDA Toolkit 12.0 or newer");
