...
namespace cufftdx {
    namespace detail {
        struct empty_workspace {
            ...
            #ifndef __CUDACC_RTC__
            static __forceinline__ __host__ empty_workspace create(cudaError_t& error_code, cudaStream_t) noexcept {
        ...
        struct unknown_workspace {
            ...
            // __host__ functions should not be visible for NVRTC
            #ifndef __CUDACC_RTC__
            static __forceinline__ __host__ unknown_workspace create(cudaError_t& error_code, cudaStream_t) noexcept {
                error_code = cudaSuccess;
        ...
        namespace bluestein_workspace_impl {
            template<unsigned int BlueFFTSize,
                     unsigned int BlueFFTMaxThreadsPerBlock,
                     unsigned int SMValue,
                     unsigned int FFTSize,
                     class FFTComplexType>
            __global__ __launch_bounds__(BlueFFTMaxThreadsPerBlock) void kernel(FFTComplexType* w_time,
                                                                                FFTComplexType* w_freq,
                                                                                const double    theta0) {
                using bluestein_fft_type =
                    typename CUFFTDX_STD::decay<decltype(Size<BlueFFTSize>() +
                                                         Direction<fft_direction::forward>() + Precision<double>() +
                                                         Type<fft_type::c2c>() + Block() + SM<SMValue>())>::type;
                static_assert(bluestein_fft_type::storage_size == bluestein_fft_type::elements_per_thread,
                              "Workspace generation incorrectly configured");

                using blue_complex_type = typename bluestein_fft_type::value_type;

                static constexpr auto blue_fft_size = size_of<bluestein_fft_type>::value;

                blue_complex_type thread_w_time[bluestein_fft_type::storage_size];
                // Generate w_time signal and store
                const unsigned int stride        = blue_fft_size / bluestein_fft_type::elements_per_thread;
                unsigned int       index         = threadIdx.x;
                unsigned int       compute_index = index;
                for (unsigned int i = 0; i < bluestein_fft_type::elements_per_thread; i++) {
                    if (index >= FFTSize) {
                ...
                bluestein_fft_type().execute(thread_w_time, shared_mem);
        ...
        public:
            ...
            // __host__ functions should not be visible for NVRTC
            #ifndef __CUDACC_RTC__
            static __forceinline__ __host__ bluestein_workspace create(cudaError_t& error_code, cudaStream_t stream) noexcept {
                error_code = cudaSuccess;
                bluestein_workspace ws;

                constexpr auto fft_size              = FFTSize;
                constexpr auto fft_blue_size         = detail::get_bluestein_size(FFTSize);
                constexpr auto ws_buffers_size_bytes = fft_blue_size * sizeof(value_type);
                ...
                const void* kernel = (const void*)bluestein_workspace_impl::kernel<
                    fft_blue_size, bluestein_fft_type::max_threads_per_block, SMValue, FFTSize, FFTValueType>;
                error_code = cudaFuncSetAttribute(
                    kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, bluestein_fft_type::shared_memory_size);
                if (error_code != cudaSuccess) {
                    CUFFTDX_DETAIL_CUDA_CHECK_AND_EXIT(error_code);
                    return ws;
                }
...
#ifndef M_PI
    #define CUFFTDX_DETAIL_M_PI_DEFINED
    #define M_PI 3.14159265358979323846
#endif
                double theta0 = M_PI / fft_size;
#ifdef CUFFTDX_DETAIL_M_PI_DEFINED
    #undef M_PI
    #undef CUFFTDX_DETAIL_M_PI_DEFINED
#endif
                void*  args[] = {&tmp_w_time, &tmp_w_freq, &theta0};
                dim3   blocks(1, 1, 1);
                error_code = cudaLaunchKernel(
                    kernel, blocks, bluestein_fft_type::block_dim, args, bluestein_fft_type::shared_memory_size, stream);
                if (error_code != cudaSuccess) {
                    CUFFTDX_DETAIL_CUDA_CHECK_AND_EXIT(error_code);
                    return ws;
                }
        ...
        template<unsigned int FFTSize,
                 ...
                 real_mode     FFTRealMode,
                 experimental::code_type FFTCode>
        struct workspace_selector<FFTSize, FFTPrecisionType, FFTValueType, FFTDirection, FFTType, FFTSM, FFTRealMode, FFTCode, false> {
            using type = unknown_workspace;
        };

        namespace workspace_selector_impl {
        ...
        template<unsigned int FFTSize,
                 ...
                 experimental::code_type FFTCode
                 >
        struct workspace_selector<FFTSize, FFTPrecisionType, FFTValueType, FFTDirection, FFTType, FFTSM, FFTRealMode, FFTCode, true> {
            using type = typename workspace_selector_impl::helper<
                get_fft_execution_size(FFTRealMode, FFTSize),
                FFTValueType,
                FFTSM,
                is_bluestein_required<FFTSize, FFTPrecisionType, FFTDirection, FFTType, FFTSM, FFTRealMode, FFTCode>::value>::type;
        };
    } // namespace detail
...
#ifndef __CUDACC_RTC__
    template<class FFT>
    auto make_workspace(cudaError_t& error_code, cudaStream_t stream = 0) noexcept //
        ...
        return workspace_type::create(error_code, stream);
    }

    template<class FFT>
    auto make_workspace(cudaStream_t stream = 0) -> typename FFT::host_workspace_type {
        cudaError_t error_code = cudaSuccess;
        auto        ws         = make_workspace<FFT>(error_code, stream);
        if (error_code != cudaSuccess) {
